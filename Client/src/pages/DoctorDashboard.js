...
export default function DoctorDashboard() {
  ...
  const setAvailability = async () => {
    try {
      ...
      setModalTitle("Error");
      setModalMessage(
        err.response?.data?.msg || "Failed to set availability."
      );
      setModalType("error");
      setIsModalOpen(true);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6 lg:p-8">
      <div className="container mx-auto bg-white rounded-xl shadow-2xl p-6 sm:p-8 space-y-8 sm:space-y-10">
        <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-center text-gray-800 mb-6 sm:mb-10">
          Doctor Dashboard
        </h2>

        {user && (
          <div className="bg-purple-50 p-4 sm:p-6 rounded-lg shadow-inner flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6">
            <img
              src={
                user.profileImage ||
                "https://placehold.co/100x100/aabbcc/ffffff?text=DR"
              }
              alt={user.name || user.email}
              className="w-24 h-24 sm:w-28 sm:h-28 rounded-full object-cover border-4 border-purple-300 shadow-lg"
              onError={(e) => {
            ...
            <div className="text-center md:text-left">
              ...
              <p className="text-lg sm:text-xl text-purple-600 font-semibold mb-2">
        ...
        <div className="bg-blue-50 p-4 sm:p-6 rounded-lg shadow-inner">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setAvailabilityForm({
                    ...availabilityForm,
                    patientDuration: +e.target.value,
                  })
                }
              />
            </div>
          ...
          <button
            ...
            className="w-full bg-blue-600 text-white p-3 sm:p-4 rounded-lg font-bold text-lg sm:text-xl hover:bg-blue-700 transition duration-300 shadow-md transform hover:-translate-y-1"
          >
            Submit Availability
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-10">
          <div className="bg-purple-50 p-4 sm:p-6 rounded-lg shadow-inner">
            <h3 className="text-2xl sm:text-3xl font-bold text-purple-700 mb-4 sm:mb-6 text-center">
              Your Appointments Calendar
            </h3>
            <Calendar
            ...
            {appointmentsForSelectedDate.length > 0 ? (
              <ul className="space-y-4">
                {appointmentsForSelectedDate.map((a) => (
                  <li
                    key={a._id}
                    className="bg-white p-3 sm:p-4 rounded-lg shadow-md flex flex-col sm:flex-row sm:justify-between sm:items-center transition duration-300 hover:scale-[1.02]"
                  >
